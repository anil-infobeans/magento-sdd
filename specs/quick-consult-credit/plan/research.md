...
**Purpose**: Resolve all `NEEDS CLARIFICATION` markers from the Technical Context and establish concrete technical decisions for Phase 1 design. Business-level ambiguities are tracked in [clarifications.md](../clarifications.md); as of the 2026-09-16 `/speckit-clarify` session, all 15 items (including CLA-004 and CLA-012, previously open at the time this document was first authored) are resolved. This document has been updated to reflect those final decisions rather than the interim "recommended pattern pending sign-off" language used before resolution.
...
**Rationale**: Directly satisfies [data-integrity-and-concurrency.md](../non-functional/data-integrity-and-concurrency.md) QCC-DATA-001/003/006 and Technical Architecture §9's requirement that "two parallel redeems must not both succeed against the same pre-transaction balance."
...
**Rationale**: Directly implements [credit-purchase-posting.md](../functional/credit-purchase-posting.md) QCC-PURCHASE-001/009/010/011 and Technical Architecture §7.2's recommendation; order-item granularity matches [clarifications.md](../clarifications.md) CLA-010's default interpretation.
...
## 6. REST API idempotency (CLA-004 — resolved: no dedicated mechanism)

**Decision**: The create-transaction endpoint implements **no** dedicated request-idempotency/deduplication mechanism. There is no client-supplied idempotency key/reference field in the request or response contract (see [contracts/rest-api.md](../contracts/rest-api.md)). Each request is validated and processed independently against the balance at the time it is received (standard amount/balance/authorization checks per [credit-rest-api.md](../functional/credit-rest-api.md) QCC-API-013). A replayed or retried request MAY be accepted as an additional transaction if it individually passes validation. This is a final, resolved specification decision ([clarifications.md](../clarifications.md) CLA-004, resolved 2026-09-16 via /speckit-clarify session), not a pending implementation choice.

**Rationale**: The business/API-architecture owner explicitly decided against introducing an idempotency-key contract for this release, accepting the risk that a client-side retry may cause an additional debit if the retried request itself is valid. This explicitly narrows SRS §7.2's general duplicate-prevention expectation for this interface only; it does not affect purchase-posting idempotency (§5 above, which remains a hard uniqueness constraint) or the balance-never-negative/no-double-spend invariants (QCC-DATA-001/006), which still apply to every individual request, including replays.

**Alternatives considered (rejected)**: An optional client-supplied `request_reference` field enforced as unique at the persistence layer — this was the interim recommendation prior to CLA-004's resolution and is no longer part of the design; a mandatory `Idempotency-Key` header — also not adopted, for the same reason; content-hash-based deduplication (customer + amount + message + short time window) — rejected as probabilistic and liable to reject legitimate rapid repeat redemptions.

**Implementation impact**: No `source_reference`/`request_reference`-style uniqueness column or check is required on the create-transaction path (distinct from the purchase-posting `source_reference` uniqueness in §5, which remains required). No client-facing documentation should imply replay protection exists for this endpoint.
...
**Rationale**: Matches the resolved clarification and Technical Architecture §11; keeps REST controllers as thin translation layers with no business logic (Constitution Principle I; [data-integrity-and-concurrency.md](../non-functional/data-integrity-and-concurrency.md) QCC-DATA-008).

**Alternatives considered**: SRS-variant paths (`/V1/customers/.../consult-credit-*`) — explicitly superseded by the resolved clarification; retained in [credit-rest-api.md](../functional/credit-rest-api.md) only for traceability.
...
**Rationale**: Satisfies [customer-dashboard.md](../functional/customer-dashboard.md) QCC-CUSTOMER-001/005/006 and Constitution Principle V (UX consistency via pattern reuse).
...
**Rationale**: Matches Technical Architecture §13's explicit recommendation and [admin-credit-management.md](../functional/admin-credit-management.md) QCC-ADMIN-001; avoids unnecessary scope expansion (Constitution Principle XII).
...
**Rationale**: Satisfies [security-and-access-control.md](../non-functional/security-and-access-control.md) QCC-SEC-002/003 for least-privilege administrative access.
...
**Rationale**: Directly matches [testing-and-acceptance.md](../non-functional/testing-and-acceptance.md) test-layer coverage table and Constitution Principle IV.
...
## 13. Performance target (CLA-012 — resolved: no formal SLA required)

**Decision**: No numeric SLA is required for this release ([clarifications.md](../clarifications.md) CLA-012, resolved 2026-09-16 via /speckit-clarify session — an explicit decision, not an unresolved gap). The design nonetheless applies efficient-by-default practices as sound engineering independent of this decision: O(1)-style balance reads (materialized balance row, no ledger replay), indexed, paginated ledger queries (composite `customer_id, created_at, entity_id` index per Technical Architecture §7.2), and single-transaction writes.

**Rationale**: Satisfies Constitution Principle VI's guidance to avoid both premature optimization and unnecessary inefficiency, without inventing an arbitrary number that the resolved clarification explicitly declined to set.

**Alternatives considered (rejected)**: Asserting a placeholder SLA (e.g., "200ms p95") — rejected; the resolved clarification explicitly established that no SLA is required, not merely that one is undefined.
...
| Item | Resolution |
|---|---|
| Performance Goals | No SLA required for this release — a final, resolved decision (CLA-012), not an open item |
| API replay/idempotency mechanism | No dedicated mechanism is implemented — a final, resolved decision (CLA-004), not an open item |
